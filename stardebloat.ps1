...
# stardebloat depends on Windows PowerShell 5.1 cmdlets (the Appx module's Get-AppxPackage /
...
if ($PSVersionTable.PSEdition -eq 'Core') {
    Write-Host "stardebloat requires Windows PowerShell 5.1, but it is running under PowerShell $($PSVersionTable.PSVersion) (pwsh / Core edition)." -ForegroundColor Red
...
if (-not $isAdmin) {
    Write-Host "stardebloat must be run as Administrator." -ForegroundColor Red
...
$script:DefaultLogPath = Join-Path $logsPath 'stardebloat.log'
...
if ($ExecutionContext.SessionState.LanguageMode -ne "FullLanguage") {
    Write-Error "stardebloat is unable to run on your system, powershell execution is restricted by security policies"
...
# Display ASCII art launch logo in CLI (Death Star Theme)
$prevCP = [Console]::OutputEncoding
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$null = cmd /c chcp 65001
$ESC = [char]27
Write-Host ""
Write-Host "$ESC[91m" -NoNewline
Write-Host '⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⣀⠀⪤⦤⦤⪤⪤⪤⢤⢄⣀⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀'
Write-Host '⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⦠⠴⢂⠉⠁⣀⢠⡊⠀⠀⠀⠀⢈⢻⣤⣀⢈⢉⣐⢢⢄⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀'
Write-Host '⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⦴⢊⠉⠁⠀⠀⢂⢄⠀⠀⠀⠀⠀⠀⠀⠀⢠⣶⢔⡎⣒⢶⣤⠀⠉⧑⪦⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀'
Write-Host '⠀⠀⠀⠀⠀⠀⠀⣠⢾⠉⠀⠀⠀⠀⧀⢠⢆⠀⠀⠀⠀⠀⠀⠀⢘⠀⠀⢾⣦⢘⢷⦸⢢⢲⧀⢑⢄⠉⢳⣤⠀⠀⠀⠀⠀⠀⠀'
...
Write-Host ('⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢈⠉⠉⢙⢛⢛⢛⢛⢛⢋⠉⠉⢁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀' + "$ESC[0m")
Write-Host ""
Write-Host "             stardebloat is launching..." -ForegroundColor Red
Write-Host "                Keep this window open" -ForegroundColor Gray
Write-Host ""
Write-Host ""
# Log script output to 'stardebloat.log' at the specified path
if ($LogPath -and (Test-Path $LogPath)) {
    Start-Transcript -Path (Join-Path $LogPath 'stardebloat.log') -Append -IncludeInvocationHeader -Force | Out-Null
}
else {
...
try {
    ...
    if ($null -ne $computerSystem -and $computerSystem.PartOfDomain) {
        Write-Warning "This machine is domain-joined. Group Policy may override changes made by stardebloat."
    }
}
catch { }
...
if (-not ((Test-Path $script:DefaultSettingsFilePath) -and (Test-Path $script:AppsListFilePath) -and (Test-Path $script:RegfilesPath) -and (Test-Path $script:AssetsPath) -and (Test-Path $script:AppSelectionSchema) -and (Test-Path $script:ApplyChangesWindowSchema) -and (Test-Path $script:SharedStylesSchema) -and (Test-Path $script:BubbleHintSchema) -and (Test-Path $script:RestoreBackupWindowSchema) -and (Test-Path $script:FeaturesFilePath))) {
    Write-Error "stardebloat is unable to find required files, please ensure all script files are present"
    Write-Output ""
    Write-Output "Press any key to exit..."
    $null = [System.Console]::ReadKey()
    Exit
...
if (-not $script:WingetInstalled -and -not $Silent) {
    Write-Warning "WinGet is not installed or outdated, this may prevent stardebloat from removing certain apps"
    Write-Output ""
...
# Hide progress bars for app removal, as they block stardebloat's output
...
if ($script:Params.ContainsKey("Sysprep")) {
    ...
    if ($WinVersion -lt 22000) {
        Write-Error "stardebloat Sysprep mode is not supported on Windows 10"
        AwaitKeyToExit
    }
}
