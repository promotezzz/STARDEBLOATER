...
function ReplaceStartMenu {
    ...
    $backupFileName = "stardebloat-StartBackup-$timestamp.bak"
...
<#
    ...
    full path to the most recent stardebloat-StartBackup-*.bak file, or
...
function Get-StartMenuBackupPath {
    ...
    if ($Scope -eq 'CurrentUser') {
        ...
        $latestBackup = Get-ChildItem -Path (Join-Path $localStateDir 'stardebloat-StartBackup-*.bak') -ErrorAction SilentlyContinue |
        ...
        $usersStartMenuPaths = Get-ChildItem -Path $userPathString -ErrorAction SilentlyContinue
        foreach ($startMenuPath in $usersStartMenuPaths) {
            $latestBackup = Get-ChildItem -Path (Join-Path $startMenuPath.FullName 'stardebloat-StartBackup-*.bak') -ErrorAction SilentlyContinue |
                Sort-Object Name -Descending |
                Select-Object -First 1
            if ($latestBackup) { return $latestBackup.FullName }
        }
...
<#
    ...
    finds the latest stardebloat-StartBackup-*.bak file.
    ...
    .EXAMPLE
    RestoreStartMenuFromBackup -StartMenuBinFile "$env:LOCALAPPDATA\Packages\Microsoft.Windows.StartMenuExperienceHost_cw5n1h2txyewy\LocalState\start2.bin" -BackupFilePath "C:\Backups\stardebloat-StartBackup-20260101_120000.bak"
#>
function RestoreStartMenuFromBackup {
    ...
    $backupBinFile = if ([string]::IsNullOrWhiteSpace($BackupFilePath)) {
        ...
        $startMenuDir = Split-Path $StartMenuBinFile -Parent
        $latestBackup = Get-ChildItem -Path (Join-Path $startMenuDir 'stardebloat-StartBackup-*.bak') -ErrorAction SilentlyContinue |
            Sort-Object Name -Descending |
            Select-Object -First 1
    ...
    $restoreBackupFileName = "stardebloat-StartRestore-$restoreTimestamp.bak"
...
<#
    ...
    .EXAMPLE
    RestoreStartMenu -BackupFilePath "C:\Backups\stardebloat-StartBackup-20260101_120000.bak"
#>
...
<#
    ...
    .PARAMETER BackupFilePath
    Path to the backup file to restore from. If omitted, automatically
    finds the latest stardebloat-StartBackup-*.bak in each user's
    LocalState folder.
    ...
    .EXAMPLE
    RestoreStartMenuForAllUsers -BackupFilePath "C:\Backups\stardebloat-StartBackup-20260101_120000.bak"
#>
