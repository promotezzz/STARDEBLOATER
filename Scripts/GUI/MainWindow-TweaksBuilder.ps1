...
function Build-DynamicTweaks {
    ...
    function GetWikiUrlForCategory($category) {
        if (-not $category) { return 'https://github.com/Raphire/stardebloat/wiki/Features' }
        ...
        return "https://github.com/Raphire/stardebloat/wiki/Features#$slug"
    }
                    ...
                    if ($featureMap.ContainsKey($soleFid)) {
                        ...
                        if ($soleFeature.ToolTip -or $soleFeature.DisableWhenApplied -eq $true) {
                            ...
                            if ($soleFeature.DisableWhenApplied -eq $true) {
                                $tooltipText = "This tweak is already applied and cannot be undone automatically. Visit the stardebloat wiki for instructions on how to manually revert this change."
                            }
