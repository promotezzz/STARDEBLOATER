...
if ($ExecutionContext.SessionState.LanguageMode -ne "FullLanguage") {
   Write-Host "Error: stardebloat is unable to run on your system. PowerShell execution is restricted by security policies" -ForegroundColor Red
   Write-Output ""
...
Clear-Host
Write-Output "-------------------------------------------------------------------------------------------"
Write-Output " stardebloat Script"
Write-Output "-------------------------------------------------------------------------------------------"

$tempRootPath = $env:TEMP
$tempWorkPath = Join-Path $tempRootPath 'stardebloat'
$tempArchivePath = Join-Path $tempRootPath 'stardebloat.zip'

Write-Output "> Downloading stardebloat..."
...
# Unzip archive to stardebloat folder
...
Write-Output ""
Write-Output "> Launching stardebloat..."
...
# Run stardebloat script with the provided arguments
$debloatScriptPath = Join-Path $tempWorkPath 'stardebloat.ps1'
...
if (Test-Path $tempWorkPath) {
    ...
    # Cleanup, remove stardebloat directory
    Get-ChildItem -Path $tempWorkPath -Exclude Config,Logs,Backups | Remove-Item -Recurse -Force
}
