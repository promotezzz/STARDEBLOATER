...
We welcome contributions from the community. You can contribute to stardebloat by:

- Reporting issues and bugs [here](https://github.com/Raphire/stardebloat/issues/new?template=bug_report.yml)
- Submitting feature requests [here](https://github.com/Raphire/stardebloat/issues/new?template=feature_request.yml)
- Testing stardebloat
...
You can help us test the latest changes and additions to the script. If you encounter any issues, please report them [here](https://github.com/Raphire/stardebloat/issues/new?template=bug_report.yml).

> [!WARNING]
> The prerelease version of stardebloat is meant for developers to test the script. Don't use this in production environments!

You can launch the prerelease version of stardebloat by running this command:
...
2. **Clone the repository** to your local machine:
   ...
   ```powershell
   git clone https://github.com/YOUR-USERNAME/stardebloat.git
   cd stardebloat
   ```
...
3. Navigate to your stardebloat directory
4. Run the script:
...
```text
stardebloat/
â”œâ”€â”€ stardebloat.ps1             # Main PowerShell script
â”œâ”€â”€ Run.bat                      # Batch launcher for the quick launch method
â”œâ”€â”€ Scripts/                     # Additional PowerShell scripts and functions
â”‚   â”œâ”€â”€ Get.ps1                  # Script used for the quick launch method to automatically download and run stardebloat
â”‚   â”œâ”€â”€ AppRemoval/              # App package removal logic
â”‚   â”œâ”€â”€ CLI/                     # Command-line interface helpers
â”‚   â”œâ”€â”€ Features/                # Feature apply/undo logic (e.g. InvokeChanges.ps1, ReplaceStartMenu.ps1)
â”‚   â”œâ”€â”€ FileIO/                  # File input/output helpers
â”‚   â”œâ”€â”€ GUI/                     # GUI window definitions and logic
â”‚   â”œâ”€â”€ Helpers/                 # Shared helper functions
â”‚   â””â”€â”€ Threading/               # Threading utilities
â”œâ”€â”€ Config/
â”‚   â”œâ”€â”€ Apps.json                # List of supported apps for removal
â”‚   â”œâ”€â”€ DefaultSettings.json     # Default configuration preset
â”‚   â”œâ”€â”€ Features.json            # All features with metadata
â”‚   â””â”€â”€ LastUsedSettings.json    # Last used configuration (generated during use)
â”œâ”€â”€ Regfiles/                    # Registry files for all features
â”‚   â”œâ”€â”€ Undo/                    # Registry files for reverting features
â”‚   â””â”€â”€ Sysprep/                 # Registry files for Sysprep mode
â”œâ”€â”€ Schemas/                     # XAML Schemas for GUI elements
â”œâ”€â”€ Assets/                      # Static assets (icons, start menu templates)
â”œâ”€â”€ Backups/                     # Registry backups (generated during use)
â””â”€â”€ Logs/                        # Script logs (generated during use)
```
...
1. **Forgetting Get.ps1**: When adding a new command-line parameter, contributors often remember to add it to `stardebloat.ps1` but forget to add the same parameter to `Scripts/Get.ps1`. Both files **must** have matching parameters.
...
To add a new app that can be removed via stardebloat:
...
2. **Edit `Config/Apps.json`**: Add a new entry to the `"Apps"` array:
   ...
   **Field Descriptions**:
   ...
   - `Recommendation`: Indicates how strongly the app is recommended for removal. One of:
     - `safe` â€” safe to remove for most users
     - `optional` â€” can be safely removed if the user doesn't need the app
     - `unsafe` â€” should only remove if the user knows what they are doing
   - `RemovalMethod`: The method used to remove the app. One of:
     - `Appx` â€” remove as a standard Appx package via `Remove-AppxPackage` (most apps)
     - `WinGet` â€” remove via WinGet (`winget uninstall`). Use for non-Appx apps such as Microsoft Copilot.
...
- `FeatureId`: Unique identifier, this must match parameter name in the stardebloat.ps1 and Get.ps1 files.
...
Add a corresponding parameter to both `stardebloat.ps1` AND `Scripts/Get.ps1`, the parameter name should match the FeatureId you have defined in `Features.json`. In most cases this will be a switch parameter, example:
...
The default preset (`Config/DefaultSettings.json`) defines which features are automatically applied when users run stardebloat in "Default Mode" or with the `-RunDefaults` parameter. This preset should include features that are widely considered to improve the Windows experience without breaking functionality.
...
3. **Create a Pull Request** on GitHub:
   ...
   - Go to the original stardebloat repository
...
- Open a [discussion](https://github.com/Raphire/stardebloat/discussions)
